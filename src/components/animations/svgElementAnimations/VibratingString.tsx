...
export default function VibratingStrings(props: { moving?: boolean }) {
	const sPoints = [ [ 0.2, -0.9 ], [ -0.6, -0.7 ], [ 0.6, 0.6 ], [ -0.2, 0.9 ] ] as Points;

	return (
		<g>
			{_.range(6).map((n) => (
				<g transform={`translate(${n / 10}, 0)`}>
					<VibratingString moving={props.moving} points={sPoints} density={80} />
				</g>
			))}
		</g>
	);
...
function useVibrationsAroundCenter(
	...
	return to([ waveOffsets, center ] as [typeof waveOffsets, typeof center], (waveOffsets, center) =>
		applyAmplitudeCurve(center, waveOffsets)
	);
}
